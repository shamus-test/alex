...
<div class="box block">
	...
	<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std form-horizontal" id="add_address">
		...
		{foreach from=$ordered_adr_fields item=field_name}
			...
			{if $field_name eq 'phone'}
				...
				{if isset($one_phone_at_least) && $one_phone_at_least}
					...
					<div class="inline-infos required"><label class="col-sm-offset-4 col-sm-6 col-md-6 col-md-offset-4">** {l s='You must register at least one phone number.'}</label></div>
				{/if}
		...
		{if !$mobilePhoneExist}
			<div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
				<label class="control-label col-sm-4 col-sm-4" for="phone_mobile">{l s='Mobile phone'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>**</sup>{/if}</label>
				<div class="col-sm-6 col-md-6">
				<input class="validate form-control" data-validate="{$address_validation.phone_mobile.validate}" type="tel" id="phone_mobile" name="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{else}{if isset($address->phone_mobile)}{$address->phone_mobile|escape:'html':'UTF-8'}{/if}{/if}" />
				</div>
			</div>
...
<ul class="footer_links clearfix">
	<li class="pull-left">
		<a href="{$link->getPageLink('addresses', true)|escape:'html':'UTF-8'}">
			<span><i class="icon-user"></i> {l s='Back to your addresses'}</span>
		</a>
	</li>
</ul>
{strip}
