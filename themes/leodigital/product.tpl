...
{if $errors|@count == 0}
	...
	<div class="primary_block block" itemscope itemtype="http://schema.org/Product">
		...
		<div class="pb-left-column col-xs-12 col-sm-4 col-md-7">
			...
			<div id="image-block" class="clearfix">
				...
				{if $PS_STOCK_MANAGEMENT}
					<span class="last_quantities" id="last_quantities"{if ($product->quantity > $last_qties || $product->quantity <= 0) || $allow_oosp || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none"{/if} ><span class="sale-label">{l s='Still 1'}</span></span>
				{/if}
		...
		<!-- end left infos--> 
		<!-- center infos -->
		<div class="pb-center-column hidden col-xs-12 col-sm-4 col-md-2">
			...
			<form id="buy_block" {if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product">
					...
					<div class="product_attributes clearfix">
						<!-- quantity wanted -->
						{if !$PS_CATALOG_MODE}
						<p class="hidden" id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
						...
						{if isset($groups)}
							...
							<div class="product-guide">	
								...
								{foreach from=$attachments item=attachment name=attachements}		
									<a id="guide" href="{$link->getPageLink('attachment', true, NULL, "id_attachment={$attachment.id_attachment}")|escape:'html':'UTF-8'}">{$attachment.name|escape:'html':'UTF-8'}</a>
								{/foreach}
							</div>
						{/if}
					...
					<div class="box-cart-bottom">
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
						 ...
						 <p class="easy-return"><a href="{$link->getCMSLink('10')|escape:'html'}">{l s="Easy Return"}</a></p>
						 <p class="authenticity-guaranteed"><a href="{$link->getCMSLink('11')|escape:'html'}">{l s="Authenticity </p>Guaranteed"}</a>
						 <p class="free-shipping"><a href="{$link->getCMSLink('9')|escape:'html'}">{l s="Free Shipping from 2000 pesos"}</a></p>

						<div class="desc-accordian">
							<div class="row">
								<div class="col-xs-12 col-sm-12 col-md-12">
									<div id="accordion">
									  <h4 class="accordion-toggle">{l s='Editor\'s Note'}</h4>
									  <div class="accordion-content default">
									    {hook h='Alexproductextra' key=1 id_product=$product->id}
									  </div>
									  <h4 class="accordion-toggle">{l s='Description'}</h4>
									  <div class="accordion-content">
									    {$product->description}
									  </div>
									  <h4 class="accordion-toggle">{l s='Shipping and Returns'}</h4>
									  <div class="accordion-content">
									    {hook h='Alexproductextra' key=2 id_product=$product->id}
									  </div>
									</div>
								</div>
							</div>
