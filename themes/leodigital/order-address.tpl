...
<div class="addresses clearfix">
	...
	<div class="row">
		<div class="col-xs-12 col-sm-6 col-md-6"{if $cart->isVirtualCart()} style="display:none;"{/if}>
			<ul class="address item box block" id="address_delivery">
			</ul>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6">
			<ul class="address alternate_item{if $cart->isVirtualCart()} full_width{/if} box block" id="address_invoice">
			</ul>
		</div>
	</div> <!-- end row -->
...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button btn btn-default">
					<span><i class="icon-chevron-left"></i>
					{l s='Continue Shopping'}</span>
				</a>
				<button type="submit" name="processAddress" class="button btn btn-default pull-right">
