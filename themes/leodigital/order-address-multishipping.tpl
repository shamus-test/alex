...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				{if $back}
					<a href="{$link->getPageLink('order', true, NULL, "step=0&amp;back={$back}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn btn-default button"><span><i class="icon-chevron-left"></i>{l s='Continue Shopping'}</span></a>
				{else}
					<a href="{$link->getPageLink('order', true, NULL, "step=0")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn btn-default button"><span><i class="icon-chevron-left"></i>{l s='Continue Shopping'}</span></a>
				{/if}
		        <button type="submit" name="processAddress" class="button btn btn-default pull-right"><span>{l s='Proceed to checkout'}<i class="icon-chevron-right right"></i></span></button>
			</p>
		</form>
{else}
	</div>
