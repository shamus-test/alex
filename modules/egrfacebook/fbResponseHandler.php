...
if (isset($_POST['accessToken']) && !empty($_POST['accessToken']))
{
	try
	{
		/* Request Facebook to get user details with access token */
		//$user_json_encoded_result = @file_get_contents('https://graph.facebook.com/me?access_token=' .trim($_POST['accessToken']));
		$result_url = 'https://graph.facebook.com/me?access_token=' .trim($_POST['accessToken']);
		...
		if($user_json_encoded_result)
		{
			...
			if($user)
			{
				...
				if(!isset($email) or strtolower($email) =='null' or empty($email))
				 {
